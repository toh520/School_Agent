...
## 2026-09-07｜校园知识问答数据扩充与真实检索验证

- 数据扩充：新增 30 条明确标注“本地演示合成数据”的校园资料，覆盖校园卡、教务、奖助、图书馆、宿舍、信息服务、场馆、健康安全、生活服务、考试与学生事务等主题；每条均包含分类、关键词、正文和来源。
- 检索增强：RAG 索引由“标题、分类、正文”扩展为“标题、分类、关键词、来源、正文”，命中资料会去重后写入回答 `basis`，学生端可核对回答依据。
- 数据验证：Flyway V21 执行成功；30 条迁移资料全部为 `ACTIVE`。本地库另保留 1 条此前管理回归生成的 `KNOW-*` 测试资料。
- 自动化：Java Spotless/JUnit/package 36/36；Agent Ruff/格式检查与 pytest 193/193；Web TypeScript/Prettier、生产构建通过；隔离 Playwright 28/28。
- 真实问答：提问“校园卡丢失后应该如何挂失和补办？”返回 `CAMPUS_QA / COMPLETED`，命中 5 个知识片段，回答正确引用“校园卡挂失与补办指南”等资料并展示来源；对既有测试资料与新资料的时间、地点差异给出冲突提醒。
- 已知警告：前端生产包仍有既有的大于 500KB 分块提示，不影响本轮知识检索、引用或问答完成。
- 结论：校园知识内容与检索元数据均已扩充，自动化门禁和真实问答链路通过。

## 2026-09-07｜四模块稳定性回归与缺陷修复

- 范围：智能食堂、校园助手、智能图书馆、考试助手，以及 125 条人工稳定性样例中的权限、关键词、金额、库存、重复提交和故障恢复等核心风险链路。
- 全量门禁：Java Spotless/JUnit/package 36/36；Agent Ruff 与 pytest 191/191；Web TypeScript/Prettier、生产构建通过；隔离 Playwright 28/28。
- 真实链路：完整健康链路通过；真实 Playwright 9/9；菜单 58 项、购物车增删恢复、三体书名检索、算法导论 ISBN 检索、重复借阅 409、重复归还 409 均通过。
- 缺陷一：校园公告管理 Schema 遗漏 `keywords`，与需求、CSV 模板和真实测试不一致。已恢复关键词列表字段并补充规范化单测，真实公告创建、关键词搜索和停用链路通过。
- 缺陷二：同一购物车并发下单时结果为 `[200,500]`。已增加按用户事务锁、订单号随机唯一后缀、停供餐品服务端过滤和前端可恢复错误提示；复测为 `[200,409]` 且仅生成 1 个订单。
- 已知警告：前端生产包仍存在既有的大于 500KB 分块提示；不影响本轮功能、数据一致性或错误恢复测试。
- 结论：本轮发现的两项稳定性缺陷均已修复，新增回归测试生效，最终全量门禁通过。
